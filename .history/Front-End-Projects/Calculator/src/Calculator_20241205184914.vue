...
function calculate() {
    ...
    let splitString = displasplitString.join('')yUpper.value.split('');
    /* TODO 2 Find if string contains: )
    ...
    */
    ...
    if (.match(/\D+/) && splitString.length == 1) {
        console.log("Operand only")
        return "NAN";
    } if (splitString.join('').match(/\d+/) && splitString.length == 1) {
