...
const displayInner = ref("0");
// ?* Need to get rid of "0" after start®

function reset() {
    displayInner.value = "0";
    displayUpper.value = ""
}
...
function clickHandler(event) {
    ...
    if (displayUpper.value + event.target.innerText != 0) {
        ...
        event.target.classList.contains('btnAction--AC') ? (reset(),appendInput(event)) : null
    } else {
        null
    }
}
...
function appendInput(event) {
    displayInner.value += event.target.innerText;
}
...
<template>
    <div class="calculator">
        ...
        <button class="btnAction btnAction--enter" @click="clickHandler">=</button>
