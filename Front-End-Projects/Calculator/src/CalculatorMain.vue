...
function clickHandler(event) {
    console.log(displayUpper.value, displayLower.value)
    displayLower.value = "0";
    initDisplay();
    if (calculated.value.length > 0) {
        displayUpper.value = calculated.value;
        if (event.target.id == "decimal" && displayUpper.value.toString().match(/\./g).length >= 1) {
            console.log("No!", displayUpper.value)
        }
    }
...
function calculate() {
    ...
    */
    ...
    displayLower.value = math.evaluate(displayUpper.value).toPrecision(4);
    displayUpper.value = displayUpper.value + "=" + math.evaluate(displayUpper.value).toPrecision(4);
    calculated.value = math.evaluate(displayUpper.value).toPrecision(4);
}
